    ...
    @if(!$budgets->isEmpty())
        <table class="Table Table-first BackContainer capitalize">
            ...
            @foreach($budgets as $budget)
                ...
                <tr class="sibling">
                    <td>@if($budget->productProviders->count())
                            <button class="iconPlus"></button>@endif</td>
                    <td> {{$budget->created_budget}}</td>
                    <td> {{$budget->productProviders->count()}}</td>
                    <td> $ <span class="sumTotal"></span></td>
                    <td>
                ...
                <tr class="SubTable2">
                    <td colspan="5">
                        ...
                        <table class="Table">
                            ...
                            {{--*/ $i = 0 /*--}}
                            @foreach($budget->productProviders as $products)
                                <tr>
                                    <td>{{$products->product->name}}</td>
                                    <td>{{$products->pivot->quantity}}</td>
                                    <td>${{$products->price}}</td>
                                    <td>${{$products->total_value}}</td>
                                    {{--*/ $i += $products->total_value /*--}}
                                </tr>
                            @endforeach
                            <tr>
                                <input type="hidden" name="" value="{{$i}}" class="priceTotal">
                            </tr>
                            </tbody>
                        </table>
                        <div class="Table-pdf">
...
@section('scripts')
    ...
    <script>
        $('.SubTable2').each(function(i){
            $(this).prev('.sibling').find('.sumTotal').text($(this).find('.priceTotal').val())
        });
