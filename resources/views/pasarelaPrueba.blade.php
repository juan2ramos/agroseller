            ...
            @else
                ...
                @if(Auth::check())
                    @if(auth()->user()->role_id == 4)
                        ...
                        <form action="https://www.zonapagos.com/api_inicio_pago/api/inicio_pagoV2" method="POST" class="Checkout-form">
...
@section('scripts')
    <script>
        $('#finishBuy').on('click', function(){
            ...
            alert($('#descripcion_pago').val());
            var param = {
                "id_tienda" : 14992,
                "clave" : "pyp123",
                "codigo_servicio_principal" : 2701,
                "total_con_iva" : parseInt(total),
                "valor_iva" : (total * 16) / 100,
                "id_pago" : parseInt($('#id_pago').val()),
                "descripcion_pago" : $('#descripcion_pago').val(),
                "email" : $('#email').val(),
                "id_cliente" : parseInt($('#id_cliente').val()),
                "tipo_id" : parseInt($('#tipo_id').val()),
            ...
            $.ajax({
                ...
                var str = xml.documentElement,
                        re = /(?:[0-9]+){5}/g,
                        token = re.exec(new XMLSerializer().serializeToString(str));
                console.log(xml);
                console.log(token);
                //window.location.href = "https://www.zonapagos.com/t_managementpas/pago.asp?estado_pago=iniciar_pago&identificador=" + token;
            })
        });
